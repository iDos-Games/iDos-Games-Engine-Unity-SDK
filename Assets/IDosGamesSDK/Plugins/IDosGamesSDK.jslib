mergeInto(LibraryManager.library, {
    ...
    RequestPlatformAuth: function(goPtr, methodPtr) {
        var go = UTF8ToString(goPtr);
        var method = UTF8ToString(methodPtr);

        if (typeof window.requestPlatformAuth === 'function') {
            window.requestPlatformAuth(go, method);
        } else {
            console.warn("[IDOS][Auth] window.requestPlatformAuth is not defined (main-sdk.js not loaded)");
        }
    }
});
